...
using AbstractField = TrickyLib.Index.Lucene.Documents.AbstractField;
...
namespace TrickyLib.Index.Lucene.Index
{
	...
	public class CheckIndex
	{
		...
		/// Run it like this:
        /// <code>
		/// java -ea:TrickyLib.Index.Lucene... TrickyLib.Index.Lucene.Index.CheckIndex pathToIndex [-fix] [-segment X] [-segment Y]
        /// </code>
		/// <list type="bullet">
		...
		public static void  Main(System.String[] args)
		{
			...
			if (indexPath == null)
			{
				...
				System.Console.Out.WriteLine("\nUsage: java TrickyLib.Index.Lucene.Index.CheckIndex pathToIndex [-fix] [-segment X] [-segment Y]\n" + "\n" + "  -fix: actually write a new segments_N file, removing any problematic segments\n" + "  -segment X: only check the specified segments.  This can be specified multiple\n" + "              times, to check more than one segment, eg '-segment _2 -segment _a'.\n" + "              You can't use this with the -fix option\n" + "\n" + "**WARNING**: -fix should only be used on an emergency basis as it will cause\n" + "documents (perhaps many) to be permanently removed from the index.  Always make\n" + "a backup copy of your index before running this!  Do not run this tool on an index\n" + "that is actively being written to.  You have been warned!\n" + "\n" + "Run without -fix, this tool will open the index, report version information\n" + "and report any exceptions it hits and what action it would take if -fix were\n" + "specified.  With -fix, this tool will remove any segments that have issues and\n" + "write a new segments_N file.  This means all documents contained in the affected\n" + "segments will be removed.\n" + "\n" + "This tool exits with exit code 1 if the index cannot be opened or has any\n" + "corruption, else 0.\n");
				System.Environment.Exit(1);
			}
			
			if (!AssertsOn())
				System.Console.Out.WriteLine("\nNOTE: testing will be more thorough if you run java with '-ea:TrickyLib.Index.Lucene...', so assertions are enabled");
