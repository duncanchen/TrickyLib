...
using ChecksumIndexInput = TrickyLib.Index.Lucene.Store.ChecksumIndexInput;
using ChecksumIndexOutput = TrickyLib.Index.Lucene.Store.ChecksumIndexOutput;
using Directory = TrickyLib.Index.Lucene.Store.Directory;
using IndexInput = TrickyLib.Index.Lucene.Store.IndexInput;
using IndexOutput = TrickyLib.Index.Lucene.Store.IndexOutput;
using NoSuchDirectoryException = TrickyLib.Index.Lucene.Store.NoSuchDirectoryException;

namespace TrickyLib.Index.Lucene.Index
{
	...
	public sealed class SegmentInfos : List<SegmentInfo>, ICloneable
	{
		private class AnonymousClassFindSegmentsFile:FindSegmentsFile
		{
			...
			internal AnonymousClassFindSegmentsFile(SegmentInfos enclosingInstance, TrickyLib.Index.Lucene.Store.Directory Param1):base(Param1)
			{
			...
			public System.Object Run(IndexCommit commit)
			{
				...
				while (true)
				{
					...
					if (0 == method)
					{
						...
						if (files != null)
							genA = TrickyLib.Index.Lucene.Index.SegmentInfos.GetCurrentSegmentGeneration(files);
						
						TrickyLib.Index.Lucene.Index.SegmentInfos.Message("directory listing genA=" + genA);
						...
						long genB = - 1;
						for (int i = 0; i < TrickyLib.Index.Lucene.Index.SegmentInfos.defaultGenFileRetryCount; i++)
						{
							...
							{
								TrickyLib.Index.Lucene.Index.SegmentInfos.Message("segments.gen open: FileNotFoundException " + e);
								break;
							}
							catch (System.IO.IOException e)
							{
								TrickyLib.Index.Lucene.Index.SegmentInfos.Message("segments.gen open: IOException " + e);
							}
							...
							if (genInput != null)
							{
								try
								{
									...
									if (version == TrickyLib.Index.Lucene.Index.SegmentInfos.FORMAT_LOCKLESS)
									{
										long gen0 = genInput.ReadLong();
										long gen1 = genInput.ReadLong();
										TrickyLib.Index.Lucene.Index.SegmentInfos.Message("fallback check: " + gen0 + "; " + gen1);
										if (gen0 == gen1)
										{
                            ...
                            System.Threading.Thread.Sleep(new TimeSpan((System.Int64) 10000 * TrickyLib.Index.Lucene.Index.SegmentInfos.defaultGenFileRetryPauseMsec));
							
							
						}
						
						TrickyLib.Index.Lucene.Index.SegmentInfos.Message(IndexFileNames.SEGMENTS_GEN + " check: genB=" + genB);
					...
					if (1 == method || (0 == method && lastGen == gen && retry))
					{
						...
						if (genLookaheadCount < TrickyLib.Index.Lucene.Index.SegmentInfos.defaultGenLookaheadCount)
						{
							gen++;
							genLookaheadCount++;
							TrickyLib.Index.Lucene.Index.SegmentInfos.Message("look ahead increment gen to " + gen);
						}
					}
					...
					try
					{
						...
						TrickyLib.Index.Lucene.Index.SegmentInfos.Message("success on " + segmentFileName);
					...
					catch (System.IO.IOException err)
					{
						...
						TrickyLib.Index.Lucene.Index.SegmentInfos.Message("primary Exception on '" + segmentFileName + "': " + err + "'; will retry: retry=" + retry + "; gen = " + gen);
						...
						if (!retry && gen > 1)
						{
							...
							if (prevExists)
							{
								TrickyLib.Index.Lucene.Index.SegmentInfos.Message("fallback to prior segment file '" + prevSegmentFileName + "'");
								try
								{
									System.Object v = DoBody(prevSegmentFileName);
									if (exc != null)
									{
										TrickyLib.Index.Lucene.Index.SegmentInfos.Message("success on fallback " + prevSegmentFileName);
									}
									return v;
								}
								catch (System.IO.IOException err2)
								{
									TrickyLib.Index.Lucene.Index.SegmentInfos.Message("secondary Exception on '" + prevSegmentFileName + "': " + err2 + "'; will retry");
								}
							}
						}
					}
        ...
        #region TrickyLib.Index.Lucene (Equals & GetHashCode )
        /// <summary>
