...
using ReaderUtil = TrickyLib.Index.Lucene.Util.ReaderUtil;
...
namespace TrickyLib.Index.Lucene.Search
{
	...
	public class MultiSearcher:Searcher
	{
		...
		{
			public AnonymousClassCollector(TrickyLib.Index.Lucene.Search.Collector collector, int start, MultiSearcher enclosingInstance)
			{
				InitBlock(collector, start, enclosingInstance);
			}
			private void  InitBlock(TrickyLib.Index.Lucene.Search.Collector collector, int start, MultiSearcher enclosingInstance)
			{
				this.collector = collector;
				this.start = start;
				this.enclosingInstance = enclosingInstance;
			}
			private TrickyLib.Index.Lucene.Search.Collector collector;
			private int start;
		...
		public /*protected internal*/ override Weight CreateWeight(Query original)
		{
			...
			// step 2
		    ISet<Term> terms = TrickyLib.Index.Lucene.Support.Compatibility.SetFactory.CreateHashSet<Term>();
