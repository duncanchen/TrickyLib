...
namespace TrickyLib.Index.Lucene.Search
{
	...
	public class ParallelMultiSearcher : MultiSearcher/*, IDisposable*/ //No need to implement IDisposable like java, nothing to dispose with the TPL
	{
		...
		{
			public AnonymousClassCollector1(TrickyLib.Index.Lucene.Search.Collector collector, int start, ParallelMultiSearcher enclosingInstance)
			{
				InitBlock(collector, start, enclosingInstance);
			}
			private void  InitBlock(TrickyLib.Index.Lucene.Search.Collector collector, int start, ParallelMultiSearcher enclosingInstance)
			{
