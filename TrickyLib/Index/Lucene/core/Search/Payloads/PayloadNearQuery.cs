...
using ToStringUtils = TrickyLib.Index.Lucene.Util.ToStringUtils;
using Explanation = TrickyLib.Index.Lucene.Search.Explanation;
using Scorer = TrickyLib.Index.Lucene.Search.Scorer;
using Searcher = TrickyLib.Index.Lucene.Search.Searcher;
using Similarity = TrickyLib.Index.Lucene.Search.Similarity;
using Weight = TrickyLib.Index.Lucene.Search.Weight;
using NearSpansOrdered = TrickyLib.Index.Lucene.Search.Spans.NearSpansOrdered;
using NearSpansUnordered = TrickyLib.Index.Lucene.Search.Spans.NearSpansUnordered;
using SpanNearQuery = TrickyLib.Index.Lucene.Search.Spans.SpanNearQuery;
using SpanQuery = TrickyLib.Index.Lucene.Search.Spans.SpanQuery;
using SpanScorer = TrickyLib.Index.Lucene.Search.Spans.SpanScorer;
using SpanWeight = TrickyLib.Index.Lucene.Search.Spans.SpanWeight;

namespace TrickyLib.Index.Lucene.Search.Payloads
{
	
	/// <summary> This class is very similar to
	/// <see cref="TrickyLib.Index.Lucene.Search.Spans.SpanNearQuery" /> except that it factors
	/// in the value of the payloads located at each of the positions where the
	/// <see cref="TrickyLib.Index.Lucene.Search.Spans.TermSpans" /> occurs.
	/// <p/>
	/// In order to take advantage of this, you must override
	/// <see cref="TrickyLib.Index.Lucene.Search.Similarity.ScorePayload" />
	/// which returns 1 by default.
	/// <p/>
	/// Payload scores are aggregated using a pluggable <see cref="PayloadFunction" />.
	/// 
	/// </summary>
	/// <seealso cref="TrickyLib.Index.Lucene.Search.Similarity.ScorePayload">
	/// </seealso>
	[Serializable]
	public class PayloadNearQuery:SpanNearQuery, System.ICloneable
	{
		...
		public class PayloadNearSpanScorer:SpanScorer
		{
			...
			new internal TrickyLib.Index.Lucene.Search.Spans.Spans spans;
			
			protected internal float payloadScore;
			private int payloadsSeen;
			internal Similarity similarity;
			
			protected internal PayloadNearSpanScorer(PayloadNearQuery enclosingInstance, TrickyLib.Index.Lucene.Search.Spans.Spans spans, Weight weight, Similarity similarity, byte[] norms):base(spans, weight, similarity, norms)
			{
				InitBlock(enclosingInstance);
				this.spans = spans;
			}
			...
			public virtual void  GetPayloads(TrickyLib.Index.Lucene.Search.Spans.Spans[] subSpans)
			{
			...
			public /*protected internal*/ override bool SetFreqCurrentDoc()
			{
				...
				TrickyLib.Index.Lucene.Search.Spans.Spans[] spansArr = new TrickyLib.Index.Lucene.Search.Spans.Spans[1];
				spansArr[0] = spans;
				payloadScore = 0;
				payloadsSeen = 0;
