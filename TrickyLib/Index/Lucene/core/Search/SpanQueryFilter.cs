...
using OpenBitSet = TrickyLib.Index.Lucene.Util.OpenBitSet;
...
namespace TrickyLib.Index.Lucene.Search
{
	...
	/// <p/>
	/// This filter does not cache.  See the <see cref="TrickyLib.Index.Lucene.Search.CachingSpanFilter" /> for a wrapper that
	/// caches.
	/// 
	/// 
	/// </summary>
	...
	public class SpanQueryFilter:SpanFilter
	{
		...
		/// <c>query</c>.
		/// </summary>
		/// <param name="query">The <see cref="TrickyLib.Index.Lucene.Search.Spans.SpanQuery" /> to use as the basis for the Filter.
		/// </param>
		...
		public override SpanFilterResult BitSpans(IndexReader reader)
		{
			...
			TrickyLib.Index.Lucene.Search.Spans.Spans spans = internalQuery.GetSpans(reader);
