...
using DocIdSet = TrickyLib.Index.Lucene.Search.DocIdSet;
using DocIdSetIterator = TrickyLib.Index.Lucene.Search.DocIdSetIterator;

namespace TrickyLib.Index.Lucene.Util
{
	...
	/// borrowed from <see cref="TrickyLib.Index.Lucene.Store.IndexInput" /> and
	/// <see cref="TrickyLib.Index.Lucene.Store.IndexOutput" />.<p/>
	...
	public class SortedVIntList:DocIdSet
	{
		private class AnonymousClassDocIdSetIterator:DocIdSetIterator
		{
			...
			{
				// See TrickyLib.Index.Lucene.Store.IndexInput.readVInt()
				sbyte b = Enclosing_Instance.bytes[bytePos++];
				lastInt += (b & TrickyLib.Index.Lucene.Util.SortedVIntList.VB1);
				for (int s = TrickyLib.Index.Lucene.Util.SortedVIntList.BIT_SHIFT; (b & ~ TrickyLib.Index.Lucene.Util.SortedVIntList.VB1) != 0; s += TrickyLib.Index.Lucene.Util.SortedVIntList.BIT_SHIFT)
				{
					b = Enclosing_Instance.bytes[bytePos++];
					lastInt += ((b & TrickyLib.Index.Lucene.Util.SortedVIntList.VB1) << s);
				}
			}
		...
		private class SortedVIntListBuilder
		{
			...
			internal virtual void  AddInt(int nextInt)
			{
				...
				// See TrickyLib.Index.Lucene.Store.IndexOutput.writeVInt()
				while ((diff & ~ TrickyLib.Index.Lucene.Util.SortedVIntList.VB1) != 0)
				{
					// The high bit of the next byte needs to be set.
					Enclosing_Instance.bytes[Enclosing_Instance.lastBytePos++] = (sbyte) ((diff & TrickyLib.Index.Lucene.Util.SortedVIntList.VB1) | ~ TrickyLib.Index.Lucene.Util.SortedVIntList.VB1);
					diff = Number.URShift(diff, TrickyLib.Index.Lucene.Util.SortedVIntList.BIT_SHIFT);
				}
