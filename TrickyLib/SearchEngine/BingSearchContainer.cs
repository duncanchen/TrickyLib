...
namespace TrickyLib.SearchEngine
{
    ...
    public partial class WebResult
    {
        ...
        {
            string output = this.ID.ToString().ReplaceTRN()
                + " ###BR### " + this.Title.ReplaceTRN()
                + " ###BR### " + this.Description.ReplaceTRN()
                + " ###BR### " + this.DisplayUrl.ReplaceTRN()
                + " ###BR### " + this.Url.ReplaceTRN();
            return output;
        }

        public void ReadString(string str)
        {
            var items = str.Split(new string[] { " ###BR### " }, StringSplitOptions.None);
            this.ID = Guid.Parse(items[0].RecoverTRN());
            this.Title = items[1].RecoverTRN();
            this.Description = items[2].RecoverTRN();
            this.DisplayUrl = items[3].RecoverTRN();
            this.Url = items[4].RecoverTRN();
        }
    }
    ...
    {
        private const string accountKey = "ijPGwUIj+OIvHq+HHVnQWZucY6AUC0+KnrjoWqyCrGI=";
        private const string rootUrl = "https://api.datamarket.azure.com/Bing/Search";

        public static BingSearchContainer bingContainer = new BingSearchContainer(new Uri(rootUrl)) { Credentials = new NetworkCredential(accountKey, accountKey) };
        ...
        public static IEnumerable<WebResult> GetWebSearchResults(string query, int topN = 10, string market = "en-us")
        {
            var webQuery = bingContainer.Web(query, null, null, market, null, null, null, null);
            webQuery = webQuery.AddQueryOption("$top", topN);
            return webQuery.Execute();
        }

        public static IEnumerable<NewsResult> GetNewsSearchResults(string query, int topN = 10, string market = "en-us")
        {
            var newsQuery = bingContainer.News(query, null, market, null, null, null, null, null, null);
            newsQuery = newsQuery.AddQueryOption("$top", topN);
            return newsQuery.Execute();
        }

        public static List<IEnumerable<WebResult>> GetWebSearchResults(string[] queries, int topN = 10, string market = "en-us")
        {
            List<IEnumerable<WebResult>> webResults = new List<IEnumerable<WebResult>>();
            foreach (var query in queries)
            {
                var webQuery = bingContainer.Web(query, null, null, market, null, null, null, null);
                webQuery = webQuery.AddQueryOption("$top", topN);
                webResults.Add(webQuery.Execute());
            }

            return webResults;
        }

        public static List<IEnumerable<NewsResult>> GetNewsSearchResults(string[] queries, int topN = 10, string market = "en-us")
        {
            List<IEnumerable<NewsResult>> newsResults = new List<IEnumerable<NewsResult>>();
            foreach (var query in queries)
            {
                var newsQuery = bingContainer.News(query, null, market, null, null, null, null, null, null);
                newsQuery = newsQuery.AddQueryOption("$top", topN);
                newsResults.Add(newsQuery.Execute());
            }
